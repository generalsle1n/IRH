...
{
    internal class LoginAuditCommand
    {
        ...
        internal LoginAuditCommand(Logger Logger)
        {
            _logger = Logger;
        }
        ...
        internal Command CreateCommand(RootCommand RootCommand)
        {
            ...
            Command.SetAction(async parseResult =>
            {
                ...
                if (parseResult.GetValue(ExistingQuery) is not null)
                {
                    ...
                    {
                        _logger.Warning($"No Query with the Name {parseResult.GetRequiredValue(ExistingQuery)} found, is there an typo?");
                        Environment.Exit(-1);
                    }
                }else
                {
