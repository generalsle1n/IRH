...
namespace IRH.Remote.Commands.InstallMSI
{
    internal class InstallMSI
    {
        private const string _commandName = "--Install";
        private const string _commandDescription = "Install an MSI on the remote Machine";
        private const string _commandAlias = "-IN";
        ...
        private const string _msiPathName = "--Path";
        private const string _msiPathDescription = "Enter the Path to the destination msi File";
        private const string _msiPathAlias = "-PA";
        private const bool _msiPathIsRequired = true;

        private const string _optionName = "--Option";
        private const string _optionDescription = "Enter the option for the msi installtion";
        private const string _optionAlias = "-O";
        ...
        private const int _remoteMachineTimeoutDefaultValue = 60;

        private const string _optionSeperator = "-";

        private readonly Logger _logger;

        internal InstallMSI(Logger Logger)
        {
            _logger = Logger;
        }
        ...
        internal Command CreateCommand(Command Root)
        {
            ...
            Option<string> MSIPathOption = new Option<string>(name: _msiPathName, description: _msiPathDescription)
            {
                IsRequired = _msiPathIsRequired
            };
            MSIPathOption.AddAlias(_msiPathAlias);

            Option<string> OptionOption = new Option<string>(name: _optionName, description: _optionDescription);
            OptionOption.AddAlias(_optionAlias);
            ...
            Command.AddOption(MSIPathOption);
            Command.AddOption(OptionOption);
            ...
            Command.SetHandler(async (Context) =>
            {
                ...
                Option<string> MSIPathOption = ExecutionCommandResult.Command.Options.Where(id => id.Name.Equals(_msiPathName.Replace(_optionSeperator, ""))).First() as Option<string>;
                Option<string> OptionOption = ExecutionCommandResult.Command.Options.Where(id => id.Name.Equals(_optionName.Replace(_optionSeperator, ""))).First() as Option<string>;
                    ...
                    case DeploymentType.WMI:
                        ...
                        using (CimSession Session = CimHelper.CreateSession(Parser.GetValueForOption(RemoteMachineOption), Domain, Parser.GetValueForOption(RemoteMachineUserNameOption), Parser.GetValueForOption(RemoteMachinePasswordOption), Parser.GetValueForOption(RemoteMachineTimeoutOption)))
                        {
                            ...
                            if (ConnectionCheck)
                            {
                                ...
                                bool Success = ProductHelper.InstallSingleMSI(Session, Parser.GetValueForOption(MSIPathOption), Parser.GetValueForOption(OptionOption), Parser.GetValueForOption(RemoteMachineTimeoutOption), _logger);

                                if (Success)
                                {
                                    _logger.Information("Software installed successful!");
                                }
                                else
                                {
                                    _logger.Error("Software installtion failed!");
                                }
                            }
                            else
                            {
